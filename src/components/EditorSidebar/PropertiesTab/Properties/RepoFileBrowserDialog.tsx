...
interface RepoFileBrowserDialogProps {
  open: boolean;
  onClose: () => void;
  /** Allowed file extensions, e.g. [".svg", ".png"]. Accepts all files when empty/undefined. */
  accept?: string[];
  /** Currently selected repo-absolute path, used to pre-expand & highlight the tree. */
  selectedAbsPath?: string;
  onPick: (absPath: string) => void;
}

/**
 * Repo file-tree browser dialog, reused by both the top-level `RepoFileProperty`
 * editor and per-row file pickers (e.g. NavigationTabs' `TabListProperty`).
 */
const RepoFileBrowserDialog: React.FC<RepoFileBrowserDialogProps> = ({
  open,
  onClose,
  accept,
  selectedAbsPath,
  ...
  const selectedAbsPathRef = useRef(selectedAbsPath);
  const opiPathRef = useRef(opiPath);
  selectedAbsPathRef.current = selectedAbsPath;
  opiPathRef.current = opiPath;

  // Pre-expand to the current value's directory (or OPI dir) when the dialog opens.
  useEffect(() => {
    if (!open) return;
    const currentAbs = selectedAbsPathRef.current ?? opiPathRef.current;
        ...
        {!repoTree || !hasFiles ? (
          ...
          <SimpleTreeView
            ...
            onItemClick={(_, itemId) => {
              if (filePathSet.has(itemId)) onPick(itemId);
            }}
          >
...
export default React.memo(RepoFileBrowserDialog);
